...
class Program:
    ...
    def begin_closing(self):
        return self.input_len > (Closing_Threshold * Max_Input_Len)
    ...
    def choose_a_successor_state(self, pstate, states):
        ...
        if self.begin_closing():
            ...
            si_ = [i for i in si if self.range_at(states[i][1], last+1) != (m, n)]
            if si_: si = si_
        ...
        while True:
            ...
            try:
                ...
                if nsucc == 0:
                    ...
                    (pstate, state), ss = self.choose_a_successor_state(state, my_succ)
                    states.extend(ss)
